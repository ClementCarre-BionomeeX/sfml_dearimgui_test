...
Matrix fruchterman_reingold(Matrix const&            A,
                            double                   k          = -1,
                            Matrix                   pos        = Matrix(),
                            std::vector<bool> const& fixed      = std::vector<bool>(),
    ...
    double t =
        ...
        std::max(
            ...
            (*std::max_element(pos.begin(),
                ...
                (*std::min_element(pos.begin(), pos.end(), [](const Vector& a, const Vector& b) {
                    return a[1] < b[1];
                }))[1]);
        ...
        for (size_t i = 0; i < nnodes; ++i) {
            for (size_t j = 0; j < nnodes; ++j) {
                if (i != j) {
                    ...
                    delta[i][j] = std::max(delta[i][j], 0.01);
                }
            }
        }

        for (size_t i = 0; i < nnodes; ++i) {
            std::fill(displacement[i].begin(), displacement[i].end(), 0.0);
            for (size_t j = 0; j < nnodes; ++j) {
                if (i != j) {
                    Vector diff(dim);
